...
/**
 * Test harness for Pair the Numbers.
 *
...
public class TestRunner {
    ...
    private static boolean matches(int[] actual, int expectedI, int expectedJ) {
        return actual != null && actual.length == 2 && actual[0] == expectedI && actual[1] == expectedJ;
    }
    ...
    private static TierResult runTier(String tier) throws IOException {
        ...
        for (TestData.TestCase c : cases) {
            ...
            try {
                int[] actual = PairNumbers.findPair(c.numbers, c.target);
                boolean ok = matches(actual, c.expectedI, c.expectedJ);
                status = ok ? "PASS" : "FAIL";
            ...
            totalMs += elapsedMs;
            System.out.printf("[%s] %s/%s: expected=[%d, %d] actual=%s (%.1fms)%n",
                    status, tier, c.name, c.expectedI, c.expectedJ, actualStr, elapsedMs);
        }
